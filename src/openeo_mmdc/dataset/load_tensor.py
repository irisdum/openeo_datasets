...
def create_mod_df_tensor(path_dir: str, s2_tile, modality: str, format=".pt"):
    assert Path(path_dir).exists(), f"{path_dir} not found"
    list_available_sits = [
        p for p in Path(path_dir).rglob(f"*/**/*{modality}{format}")
    ]
        ...
        l_df += [
            pd.DataFrame(
                create_dict_one_sits(
                    path_sits=path, mod=modality, s2_tile=s2_tile
                )
            )
            for path in l_s2
        ]
...
def load_sits(
    ...
    crop_nan_mask = crop_spat_temp(
        sits_obj.mask.mask_nan, x, y, crop_size, temp_idx
    )
